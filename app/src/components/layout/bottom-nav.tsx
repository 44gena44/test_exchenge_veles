...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-16 md:bottom-24 left-1/2 -translate-x-1/2 z-50 w-full max-w-[430px] px-16">
        ...
        {navItems.map((item) => {
          const isActive =
            item.href === "/obmennik"
              ? pathname === item.href
              : item.includedPaths
              ? item.includedPaths.some((path) => pathname.startsWith(path))
          ...
          return (
            ...
            >
              <div
                className={cn(
                  "flex items-center justify-center w-40 h-40 rounded-full transition-all duration-300",
